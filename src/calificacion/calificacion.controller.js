...
export const generarBoletaPDF = async (req, res) => {
    try {
        ...
        // Formatear nombre para el archivo (sin espacios, mayúsculas, sin tildes)
        const sanitizarTexto = (texto) => {
            return texto
                .toUpperCase()
                .normalize('NFD')
                .replace(/[\u0300-\u036f]/g, '') // Quita tildes
                .replace(/[^A-Z0-9]/g, '_')      // Reemplaza caracteres especiales por _
                .replace(/_+/g, '_')             // Elimina múltiples guiones bajos
                .replace(/^_|_$/g, '');          // Elimina guiones al inicio/fin
        };

        const nombreArchivo = `BOLETA_${sanitizarTexto(estudiante.name)}_${sanitizarTexto(estudiante.surname)}_${ciclo}.pdf`;
